...
chcp 65001 >nul
rem ============================================================================
rem  OKU - Instalador de tareas programadas de la FLOTA (1 clic)
rem  Correr en TU PC (el que administra los restaurantes), NO en el del local.
rem  Crea dos tareas de Windows:
rem    OKU_Respaldo : respaldo diario de TODAS las bases (por defecto 09:00)
rem    OKU_Monitor  : monitor de salud de la flota cada 10 minutos
...
schtasks /Create /F /TN "OKU_Respaldo" /SC DAILY /ST %HORA_RESPALDO% /TR "\"%SCRIPTS%respaldo_diario.bat\"" >nul
if errorlevel 1 (
    echo [ERROR] No se pudo crear la tarea OKU_Respaldo.
    pause
    exit /b 1
)
echo [OK] Tarea OKU_Respaldo: respaldo diario de la flota a las %HORA_RESPALDO%.

schtasks /Create /F /TN "OKU_Monitor" /SC MINUTE /MO %MONITOR_CADA_MIN% /TR "\"%SCRIPTS%monitor_flota.bat\"" >nul
if errorlevel 1 (
    echo [ERROR] No se pudo crear la tarea OKU_Monitor.
    pause
    exit /b 1
)
echo [OK] Tarea OKU_Monitor: monitor de salud cada %MONITOR_CADA_MIN% minutos.

echo.
echo Listo. Verificacion rapida:
echo   - Corre ahora un respaldo de prueba:   schtasks /Run /TN OKU_Respaldo
