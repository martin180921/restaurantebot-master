...
# ── Accesos de mesero: PINs de turno efímeros (objetivo 2) ──────────────────────
...
def _seccion_accesos_mesero():
    st.markdown('<div class="section-title">🔑 Accesos de mesero · PIN de turno</div>',
                unsafe_allow_html=True)
    ...
    if not activas:
        st.caption("Sin accesos activos. Genera un PIN cuando un mesero entre a su turno; "
                   "se invalidan al cerrar la caja o al cerrar su turno aquí.")
        return
    for k in activas:
        kid = int(k["id"])
...
def render():
    ...
    # ── Accesos de mesero (PINs de turno) ───────────────────────────────────────
    st.divider()
    _seccion_accesos_mesero()
